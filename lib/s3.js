...
const checkOnS3 = async (changeset) => {
  ...
  try {
    ...
    elements = _.uniqWith(elements, (eltA, eltB) => {
    ...
    console.error(`ERR: Failed to read changeset from S3 (${OUTPUT_BUCKET}/${changesetId}.json): ${e.code || e.message}`);
    return changeset; // ⬅️ Continúa devolviendo el original si no encuentra
  }
};
